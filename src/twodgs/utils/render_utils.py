...
def create_videos(base_dir, input_dir, out_name, num_frames=480):
    ...
    for k in ['depth', 'normal', 'color']:
        ...
        if k == 'color':
            file0 = os.path.join(
                input_dir, 'renders', f'{idx_to_str(0)}.{file_ext}'
            )
        else:
            file0 = os.path.join(
        ...
        with media.VideoWriter(
            ...
            for idx in tqdm(range(num_frames)):
                if k == 'color':
                    ...
                    img_file = os.path.join(
                        input_dir, 'vis',
                        f'{k}_{idx_to_str(idx)}.{file_ext}'
                    )

                if not os.path.exists(img_file):
                    raise ValueError(
                        f'Image file {img_file} does not exist.'
                    )
                ...
                if k in ['color', 'normal']:
                    ...
                    img = np.clip(
                        (img - np.minimum(lo, hi)) / np.abs(hi - lo), 0, 1
                    )
                    img = cm.get_cmap('turbo')(img)[..., :3]

                frame = (
                    np.clip(np.nan_to_num(img), 0., 1.) * 255.
                ).astype(np.uint8)
...
def save_img_u8(img, pth):
    ...
    with open(pth, 'wb') as f:
        Image.fromarray(
            (np.clip(np.nan_to_num(img), 0., 1.) * 255.).astype(np.uint8)
        ).save(f, 'PNG')
...
def save_img_f32(depthmap, pth):
    ...
    with open(pth, 'wb') as f:
        Image.fromarray(
            np.nan_to_num(depthmap).astype(np.float32)
        ).save(f, 'TIFF')
