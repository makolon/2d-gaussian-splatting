...
2dgs-train -s <path to COLMAP or NeRF Synthetic dataset> 
# View the trained model
2dgs-view -s <path to COLMAP or NeRF Synthetic dataset> -m <path to trained model> 
```
...
# install the package (editable install keeps the CLI scripts up to date)
pip install -e .
# or fetch directly from GitHub in another project
pip install git+https://github.com/hbb1/2d-gaussian-splatting.git

> NOTE: Because Python identifiers cannot begin with a digit, the distribution
> is installed as `2dgs` but imported as `twodgs`.
```
## Training
...
```bash
2dgs-render -m <path to pre-trained model> -s <path to COLMAP dataset> 
```
...
```bash
2dgs-render -m <path to pre-trained model> -s <path to COLMAP dataset> --mesh_res 1024
```

## Quick Examples
Assuming you have downloaded [MipNeRF360](https://jonbarron.info/mipnerf360/), simply use
```bash
2dgs-train -s <path to m360>/<garden> -m output/m360/garden
# use our unbounded mesh extraction!!
2dgs-render -s <path to m360>/<garden> -m output/m360/garden --unbounded --skip_test --skip_train --mesh_res 1024
# or use the bounded mesh extraction if you focus on foreground
2dgs-render -s <path to m360>/<garden> -m output/m360/garden --skip_test --skip_train --mesh_res 1024
```
...
```bash
2dgs-train -s <path to dtu>/<scan105> -m output/date/scan105 -r 2 --depth_ratio 1
2dgs-render -r 2 --depth_ratio 1 --skip_test --skip_train
```
